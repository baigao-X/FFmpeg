...
typedef struct {
   ...
   /**
     * 开启异步录像后未找到流后会广播该事件，请在监听该事件后去拉流或其他方式产生流，这样就能按需拉流了
     * @param url_info 播放url相关信息
     * @return 1 直接关闭
     *         0 等待流注册
     */
    int (API_CALL *on_mk_record_media_not_found)(const mk_media_info url_info);
     ...
     *         0 等待流注册
     */
    int(API_CALL *on_mk_media_pull_stream_from_origin_failed)(const mk_media_info url_info);
